...
def part1_show_T_pose(viewer, joint_names, joint_parents, joint_offsets):
    ...
    for joint_idx, parent_idx in enumerate(joint_parents):
        ...
        #print("Offset: ", joint_offsets[joint_idx])
        #print("joint_parents: ", joint_parents)
        #print("joint_parents[joint_idx]: ", joint_parents[joint_idx])
        #print("joint_names[joint_idx]", joint_names[joint_idx])
        #print("global_joint_position[joint_idx]", global_joint_position[joint_idx])
        

        if joint_parents[joint_idx] == -1:
            global_joint_position[joint_idx] = global_joint_position[joint_idx]
        else:
            global_joint_position[joint_idx] = global_joint_position[joint_parents[joint_idx]] + joint_offsets[joint_idx]

        #print("current joint global position: ", global_joint_position[joint_idx])
        #print("\n")
...
def part2_forward_kinametic(viewer, joint_names, joint_parents, joint_offsets, joint_positions, joint_rotations, show_animation=False):
    ...
    for joint_idx, parent_idx in enumerate(joint_parents):
        ...
        rotated_vector = parent_orientation.apply(joint_positions[:, joint_idx, :])     #Q_i-1
