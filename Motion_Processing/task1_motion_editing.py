...
def interpolation(left_data, right_data, t, method='linear', return_first_key=True):
    ...
    if method == 'linear':
        ...
        for joint_idx in range(joint_num):
            ...
            new_key_frames = np.linspace(0, 1, t+1)
            interp_rots = slerp(new_key_frames).as_quat()
            res.append(interp_rots)
        res = np.array(res)
...
def concatenate_two_motions(motion1, motion2, last_frame_index, start_frame_indx, between_frames, searching_frames=20, method='interpolation'):
    ...
    # Find the closest frame in motion1 and motion 2
    search_source = motion1.local_joint_rotations[last_frame_index - searching_frames:last_frame_index + searching_frames]
    search_target = motion2.local_joint_rotations[max(0, start_frame_indx - searching_frames):start_frame_indx + searching_frames]
   
    search_source = search_source.reshape(search_source.shape[0], -1)
    search_target = search_target.reshape(search_target.shape[0], -1)
   
    sim_matrix = np.zeros((search_source.shape[0], search_target.shape[0]))
    for i in range(search_source.shape[0]):
        for j in range(search_target.shape[0]):
            sim_matrix[i, j] = np.linalg.norm(search_source[i] - search_target[j])
    ...
    motion2.local_joint_positions[real_j:] += (motion1.local_joint_positions [real_i] - motion2.local_joint_positions[real_j])
...
def main():
    ...
    part2_concatenate(viewer, between_frames=8, example=False)
